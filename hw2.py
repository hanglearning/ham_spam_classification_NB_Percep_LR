...
def process_training_data_and_extract_words(training_set_dir):

    print("Processing documents, please wait...")
    
    whole_data_set_doc_by_doc = {} # used to extract the number of the word appearances for each document in the origianl training set
    ...
    data_set_by_class_label = {} # used to extract the number of the word appearances for each class. useful in naive bayes
            ...
            for doc_entry in os.listdir(os.path.join(training_set_dir, class_entry)):
                if not doc_entry.startswith('.') and os.path.isfile(os.path.join(training_set_dir, class_entry, doc_entry)):
                    with open(os.path.join(training_set_dir, class_entry, doc_entry), 'r', encoding='utf-8', errors='ignore') as doc_file:
                        ...
                        for key in words_count_for_this_doc:
    ...
    return whole_data_set_doc_by_doc, divided_training_set, divided_validation_set, data_set_by_class_label, vocabulary_set, class_labels, stop_words, vocabulary_set_divided_training_set

try:
    whole_data_set_doc_by_doc, divided_training_set, divided_validation_set, data_set_by_class_label, vocabulary_set, class_labels, stop_words, vocabulary_set_divided_training_set = process_training_data_and_extract_words(trainig_set_dir)
except:
...
def train_navie_bayes():
    ...
    for class_label in data_set_by_class_label:
        num_of_words_occurances[class_label] = sum(data_set_by_class_label[class_label].values())
...
def classify_and_test_for_accuracy(classifier, test_on_validation=None, divided_validation_set=None, perceptron_weight_vector=None, logistic_regression_weight_vector=None):
        ...
        for class_entry in divided_validation_set:
            for doc_entry in divided_validation_set[class_entry]:
                ...
                if classifier == "Perceptron":
                    ...
                    for word in divided_validation_set[class_entry][doc_entry]:
                        if word in vocabulary_set_divided_training_set:
                            perceptron_weighted_sum_of_this_doc += perceptron_weight_vector[word] * divided_validation_set[class_entry][doc_entry][word]
                        else:
                    ...
                    for word in divided_validation_set[class_entry][doc_entry]:
                        if word in vocabulary_set_divided_training_set:
                            logistic_regression_weighted_sum_of_this_doc += logistic_regression_weight_vector[word] * divided_validation_set[class_entry][doc_entry][word]
    ...
    print("Accuracy on the {} by {} is {}/{} ({})".format(test_set_name, classifier, correctly_cassified, documents_count, correctly_cassified/documents_count)) #https://stackoverflow.com/questions/55486728/in-python-how-can-we-combine-the-formatting-mechanism-for-both-strings-and-a-per
...
logistic_regression_weight_vector = train_logistic_regression(learning_rate, best_lambda, training_iterations, whole_data_set_doc_by_doc)
...
perceptron_weight_vector = train_perceptron(learning_rate=0.003, training_iterations=50, training_data_set=whole_data_set_doc_by_doc)
